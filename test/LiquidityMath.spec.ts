...
describe('LiquidityMath', () => {
  ...
  describe('#addDelta', () => {
    ...
    it('2**128-15 + 15 overflows', async () => {
      await expect(liquidityMath.addDelta(BigNumber.from(2).pow(128).sub(15), 15)).to.be.reverted // skip reason revertedWith('LA')
    })
    it('0 + -1 underflows', async () => {
      await expect(liquidityMath.addDelta(0, -1)).to.be.reverted // skip reason revertedWith('LS')
    })
    it('3 + -4 underflows', async () => {
      await expect(liquidityMath.addDelta(3, -4)).to.be.reverted // skip reason revertedWith('LS')
    })
    it('gas add', async () => {
